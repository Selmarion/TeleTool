...
from tkinter import filedialog # Для выбора папки
...
class TeleToolApp(ctk.CTk):
    def __init__(self):
        ...
        # Путь сохранения (по умолчанию - текущая папка + downloads)
    ...
    def update_texts(self):
        t = TRANS[self.lang]
        self.title(t["app_title"].replace("v02", "v03"))
        ...
        # New Downloader fields
        self.lbl_save_path.configure(text=t["lbl_save_path"])
    ...
    def create_downloader_frame(self):
        ...
        self.lbl_save_path = ctk.CTkLabel(path_f, text="Save Path:")
        self.lbl_save_path.grid(row=0, column=0, padx=5, sticky="e")
        
        self.entry_path = ctk.CTkEntry(path_f, width=300)
        self.entry_path.grid(row=0, column=1, padx=5)
        self.entry_path.insert(0, self.download_path)
        self.entry_path.configure(state="readonly") # Защита от ручного ввода ошибок
        
        self.btn_browse = ctk.CTkButton(path_f, text="Browse...", width=80, command=self.browse_folder)
        ...
        self.btn_open_folder = ctk.CTkButton(f, text="Open Folder", fg_color="#444", hover_color="#555", command=self.open_save_folder)
    ...
    def open_save_folder(self):
        if os.path.exists(self.download_path):
            ...
            self.lbl_download_status.configure(text="Error: Folder not found")
    ...
    async def async_connect(self, api_id, api_hash):
        try:
            ...
            if not await self.client.is_user_authorized():
                self.after(0, lambda: self.lbl_status.configure(text="CHECK CONSOLE FOR CODE!", text_color="#E74C3C"))
                print("\n" + "="*40 + "\nВВЕДИТЕ КОД В КОНСОЛИ / ENTER CODE IN CONSOLE\n" + "="*40 + "\n")
    ...
    def start_download(self):
        ...
        # Передаем текущий путь сохранения
        save_folder = self.download_path
        
        downloader = GuiMediaDownloader(self.client, self.log_download, self.progress_download)
        self.run_async(self._download_wrapper(downloader, link, save_folder))

    async def _download_wrapper(self, downloader, link, save_folder):
        await downloader.download(link, save_folder)
